...
try {
    ...
    // Majuscule corrigée : Assister
    $absences_non_justifiees = $pdo->query("SELECT COUNT(*) FROM Assister WHERE Statut = 'Absent' AND Est_Justifie = 0")->fetchColumn();

    // --- 3. RAPPORT : MATIÈRES PAR FILIÈRE ---
    // Majuscule corrigée : Correspondre
    ...
    // On cherche les étudiants dont le nombre d'absences > 20% du Volume Horaire de la matière
    $rapport_exclusion = $pdo->query("
        ...
        WHERE a.Statut = 'Absent' AND a.Est_Justifie = 0
    ...
    // Si la DB plante, on stocke l'erreur pour l'afficher proprement
...
<div class="container mt-4">
    ...
    <?php if(!empty($erreur_sql)): ?>
        <div class="alert alert-danger shadow-sm no-print">
            ...
            <br><small>Vérifiez que les tables (comme Correspondre) existent bien avec ces noms exacts.</small>
        </div>
    <?php endif; ?>
    ...
    <div class="row mb-5 no-print">
        ...
        <div class="col-md-4">
            <div class="card card-stat shadow-sm bg-danger text-white p-3">
                ...
                <h2 class="fw-bold mb-0"><?= $absences_non_justifiees ?></h2>
            </div>
        </div>
    </div>
...
<!-- 4. ALERTES D'EXCLUSION (> 20% du Volume Horaire) -->
